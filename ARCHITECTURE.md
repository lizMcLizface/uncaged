...
step 1, 2026-08-03. Sections describe the *current* shape, not an
...
| `src/piano/` *(`PIANO_VIEW_PLAN.md` step 1, landed 2026-08-03 — a feature, not a refactor phase)* | The piano view. `keyModel.js`: which keys exist in a MIDI range, which are black, the white-key count `--num-keys` is set from, octave-span → MIDI range. `range.js`: the active instrument's playable range as `{ lowMidi, highMidi, openStrings }`. Both pure — no DOM, no app state. Standard MIDI (60 = C4) throughout, which is forced by `midi.js`'s `keys` table; see §6.29 for the conversion trap at the `tuning.js` boundary. | `keyModel.js`: nothing at all. `range.js`: `tuning.js` (`getNoteAtStringFret`), `theory/notation` (`noteToMidi`) | nothing imports it yet — step 2's `Piano.js` is the first consumer |
...
### 6.29 `src/piano/` (`PIANO_VIEW_PLAN.md` step 1, 2026-08-03)
